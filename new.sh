...
RANDHEX=$(dd if=/dev/urandom bs=2 count=1 2>/dev/null | od -An -t x1 | tr -d ' \n')

docker compose -p "${RANDHEX}" up -d

#Figure out what port docker mapped to localhost
PORT=$(docker port "${RANDHEX}-web-1" 80 | head -n1 | sed 's/.*://' | tr -d '\r')

# Inside the container, set the wwwroot config value to use localhost and the mapped port
MSYS_NO_PATHCONV=1 docker exec "${RANDHEX}-web-1" bash -lc "sed -Ei \"s|localhost:8080|localhost:${PORT}|g\" /var/www/html/config.php"
